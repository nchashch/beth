//! Consensus wrapper that additionally requires each block be committed in a Bitcoin mainchain
//! block per BIP301 blind merge mining, on top of the standard Ethereum checks.
//!
//! BIP300 deposit minting and withdrawal-bundle lifecycle updates are both handled via
//! [`crate::evm`]'s system calls now, applied during normal block execution — so they're
//! implicitly verified by standard state-root validation (already inherited from
//! [`EthBeaconConsensus`]) rather than needing a bespoke check here. See
//! [`crate::deposit_vault`]/[`crate::withdrawal_bundle`]'s module doc comments for the full
//! picture.
...
/// Wraps [`EthBeaconConsensus`], additionally requiring each block's hash be BIP301
/// blind-merge-mining committed in the Bitcoin mainchain block referenced by its `extraData`
/// field (see [`crate::payload`] for how that field is populated).
#[derive(Debug, Clone)]
